...
Texture2D txEmissive : register(t2);
...
float4 DoSpecular(Light lightObject, float3 vertexToEye, float3 lightDirectionToVertex, float3 Normal, float SpecularPower)
{
	...
	if (lightIntensity > 0.0f)
	{
		...
		specular = pow(saturate(dot(reflection, vertexToEye)), SpecularPower); // 32 = specular power
	}
...
LightingResult DoPointLight(Light light, float3 vertexToEye, float4 vertexPos, float3 N, float SpecularPower)
{
	...
	result.Specular = DoSpecular(light, vertexToEye, LightDirectionToVertex, N, SpecularPower) * attenuation;
...
float4 ComputeLighting(float4 vertexPos, float3 N, float4 diffuse, float3 specular, float4 emissive, float SpecularPower)
{
	...
	for (int i = 0; i < MAX_LIGHTS; ++i)
	{
		...
		result = DoPointLight(Lights[i], vertexToEye, vertexPos, N, SpecularPower);
...
float4 PS(PS_INPUT IN) : SV_TARGET
{
	...
	float2 screenSize;
	txDiffuse.GetDimensions(screenSize.x, screenSize.y);
